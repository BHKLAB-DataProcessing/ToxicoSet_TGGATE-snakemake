...
create_phenoData <- function(species = c("Human", "Rat"), verbose = TRUE) {
  ...
  # reorder columns
  all_attribute <- all_attribute[, c(
    "BARCODE", "ARR_DESIGN", "EXP_ID", "GROUP_ID", "INDIVIDUAL_ID",
  ...
  # Rename columns
  colnames(all_attribute) <- c(
    "samplename", "chiptype", "exp_id", "group_id", "individual_id", "batchid",
...
create_sensitivityProfiles <- function(phenoData, verbose = TRUE) {
  ...
  sensProf <- data.frame(
    row.names = unique(phenoData$UID),
...
create_sensitivityInfo <- function(phenoData, doseArray, verbose = TRUE) {
  ...
  for (i in unique(phenoData$UID)) {
    ...
    if ("Control" %in% c) {
      temp[i, "Control"] <- phenoData$samplename[phenoData$UID == i & phenoData$dose_level == "Control"]
    }
    if ("Low" %in% c) {
      temp[i, "Low"] <- phenoData$samplename[phenoData$UID == i & phenoData$dose_level == "Low"]
    }
    if ("Middle" %in% c) {
      temp[i, "Middle"] <- phenoData$samplename[phenoData$UID == i & phenoData$dose_level == "Middle"]
    }
    if ("High" %in% c) {
      temp[i, "High"] <- phenoData$samplename[phenoData$UID == i & phenoData$dose_level == "High"]
    }
  }
...
if (arg_select == "human") {
  ...
  saveRDS(TGGATES_humanldh, paste0(output_dir, filename))
